...
export function EnhancedBibleReader() {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-4">
      {/* Minimal Top Bar - YouVersion Style */}
      ...
      {/* Bible Text - Clean YouVersion-like layout */}
      <div className="px-4 py-6 space-y-6">
        {verses.map((verse, index) => (
          <div key={verse.id} className="group relative">
            <div className="flex gap-4">
              <div className="flex-shrink-0 mt-1">
                <span className="inline-flex items-center justify-center w-6 h-6 text-xs font-medium text-muted-foreground bg-muted/30 rounded-full">
                  {verse.number}
                </span>
              </div>
              
              <div className="flex-1 min-w-0">
                <p className={`${getFontSizeClass()} leading-relaxed text-foreground font-normal mb-3 selection:bg-primary/20`}>
                  {verse.text}
                </p>
      ...
      {/* Bottom Navigation */}
      <div className="sticky bottom-20 bg-background/95 backdrop-blur-sm px-4 py-3">
