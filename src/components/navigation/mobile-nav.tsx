...
export function MobileNav({ activeTab, onTabClick }: MobileNavProps) {
  return (
    <nav className="fixed bottom-6 left-1/2 transform -translate-x-1/2 z-50 bg-background/5 backdrop-blur-2xl shadow-glass-lg rounded-full px-6 py-3">
      <div className="flex items-center justify-center gap-1 max-w-md mx-auto">
        {navItems.map(({ id, label, icon: Icon }) => (
          <button
            ...
            className={cn(
              "flex flex-col items-center justify-center gap-1 h-16 w-16 text-xs font-medium transition-all duration-300 mobile-touch-target rounded-full backdrop-blur-md bg-transparent border-0 outline-none focus:outline-none focus:ring-0 focus:border-0 active:bg-transparent",
              activeTab === id 
